...
async def earn_money(client, message):
    ...
    if user:
        ...
        await message.reply(f"You earned {earned_money} money!")
    else:
        await message.reply("You are not registered in the game.")
...
async def spend_money(client, message):
    ...
    if user:
        ...
        if money >= 30:
            ...
            await message.reply(f"You spent {spent_money} money. Your remaining money: {money - spent_money}")
        else:
...
async def donate_money(client, message):
    ...
    if user:
        ...
        if money >= amount:
            ...
            await message.reply(f"You donated {amount} money to {receiver_username}!")
            
            # Notify support chat about the donation
            await client.send_message(SUPPORT_CHAT, f"User {user['username']} donated {amount} money to {receiver_username}.")
        else:
            await message.reply("You don't have enough money to donate.")
    else:
        await message.reply("You are not registered in the game.")

# You can add more money-related commands or functionality to this plugin as needed
